...
- (void)addScrollView {
    ...
    CGFloat height = self.frame.size.height - self.menuViewOffsetY;
    CGRect frame = CGRectMake(0, self.menuViewOffsetY, width, height);
...
- (void)calculateItemFrames {
    ...
    for (int i = 0; i < self.items.count; i++) {
        ...
        CGRect frame = CGRectMake(contentWidth, 0, itemW, self.frame.size.height - self.menuViewOffsetY);
        // 记录frame
    ...
    self.scrollView.contentSize = CGSizeMake(contentWidth, self.frame.size.height - self.menuViewOffsetY);
}
...
- (void)addProgressView {
    WMProgressView *pView = [[WMProgressView alloc] initWithFrame:CGRectMake(0, self.frame.size.height - self.menuViewOffsetY - self.progressHeight, self.scrollView.contentSize.width, self.progressHeight)];
...
- (void)addFooldViewHollow:(BOOL)isHollow {
    WMFooldView *fooldView = [[WMFooldView alloc] initWithFrame:CGRectMake(0, 0, self.scrollView.contentSize.width, self.frame.size.height - self.menuViewOffsetY)];
