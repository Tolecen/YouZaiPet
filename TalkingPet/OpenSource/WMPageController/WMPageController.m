...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    BOOL inTableViewCell = [gestureRecognizer.view isKindOfClass:[UITableViewCell class]]; // check the current touch is in table view cell
...
@property (nonatomic, weak) AllowSwipeScrollView *scrollView;
...
- (instancetype)init {
    if (self = [super init]) {
        _viewControllerClasses = [self configureViewControllerClasses];
        _titles = [self configureViewControllerTitles];
        [self setup];
    }
    return self;
}

- (UIView *)configureMenuViewSuperView {
    return self.view;
}

- (UIView *)configureScrollViewSuperView {
    return self.view;
}
...
- (void)setup {
    ...
    [self configureMenuPageProperties];
}

- (void)configureViewFrame {
    self.menuViewFrame = CGRectMake(0,
                                    0,
                                    CGRectGetWidth(self.view.frame),
                                    self.menuHeight);
    self.scrollViewFrame = CGRectMake(0,
                                      self.menuHeight,
                                      CGRectGetWidth(self.view.frame),
                                      CGRectGetHeight(self.view.frame) - self.menuHeight);
}
// 包括宽高，子控制器视图 frame
- (void)calculateSize {
...
//    // 重新计算各个控制器视图的宽高
    ...
    _viewWidth = self.scrollViewFrame.size.width;
    _viewHeight = self.scrollViewFrame.size.height;
...
- (void)addScrollView {
    AllowSwipeScrollView *scrollView = [[AllowSwipeScrollView alloc] init];
    ...
    [[self configureScrollViewSuperView] addSubview:scrollView];
    self.scrollView = scrollView;
}

- (void)addMenuView {
//    CGRect frame = CGRectMake(_viewX, _viewY, _viewWidth, self.menuHeight);
    WMMenuView *menuView = [[WMMenuView alloc] initWithFrame:self.menuViewFrame buttonItems:self.titles backgroundColor:self.menuBGColor norSize:self.titleSizeNormal selSize:self.titleSizeSelected norColor:self.titleColorNormal selColor:self.titleColorSelected];
    menuView.menuViewOffsetY = self.menuViewOffsetY;
    ...
    [[self configureMenuViewSuperView] addSubview:menuView];
...
- (void)viewDidLayoutSubviews {
    ...
    self.scrollView.contentSize = CGSizeMake(self.titles.count*_viewWidth, self.scrollViewFrame.size.height);
