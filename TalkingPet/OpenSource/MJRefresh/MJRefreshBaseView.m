...
- (UIActivityIndicatorView *)activityView
{
    if (!_activityView) {
        ...
        activityView.color = [UIColor colorWithRed:133/255.0f green:209/255.0f blue:252/255.0f alpha:1];
...
- (void)endRefreshing
{
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        ...
        if (self.canEndAnimation) {
            if (_delegate&& [_delegate respondsToSelector:@selector(animationStopAfterRefreshingFooter)]) {
                [_delegate animationStopAfterRefreshingFooter];
            }
        }
    });
}
...
//                objc_msgSend(self.beginRefreshingTaget, self.beginRefreshingAction, self);
                ((void(*)(id, SEL,id))objc_msgSend)(self.beginRefreshingTaget, self.self.beginRefreshingAction, self);
            }
