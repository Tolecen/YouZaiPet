...
- (void)initialize{
    ...
    _timeInterval = AutoScrollInterval;
    _autoScroll = YES;
...
- (void)layoutSubviews{
    ...
    if (_needsReload) {
        ...
        if (_dataSource && [_dataSource respondsToSelector:@selector(numberOfPagesInFlowView:)]) {
            ...
            if (pageControl && [pageControl respondsToSelector:@selector(setNumberOfPages:)]) {
                ...
                pageControl.hidden = (_pageCount<=1);
            }
        }
    ...
    if (_pageCount>1&&_autoScroll) {
         timer = [NSTimer scheduledTimerWithTimeInterval:_timeInterval target:self selector:@selector(autoScrollToNextPage) userInfo:nil repeats:YES];
    }
}
