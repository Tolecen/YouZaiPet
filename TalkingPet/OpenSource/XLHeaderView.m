...
//        [self addSubview:_titleLabel];
        ...
        if ([UIDevice currentDevice].systemVersion.floatValue >= 7.0){
            ...
            cSize = [subTitle sizeWithFont:[UIFont systemFontOfSize:16] constrainedToSize:CGSizeMake(ScreenWidth-20, 40) lineBreakMode:NSLineBreakByCharWrapping];
        [_subTitleLabel setFrame:CGRectMake(10, frame.size.height-10-cSize.height, cSize.width, cSize.height)];
        
        if (asNavi) {
            self.frame = CGRectMake(0, -(self.frame.size.height-64),  self.originSize.width, [UIDevice currentDevice].systemVersion.floatValue >= 7.0?self.originSize.height:0);
            self.backImageView.frame = CGRectMake(0, 0, self.frame.size.width, [UIDevice currentDevice].systemVersion.floatValue >= 7.0?self.frame.size.height:0);
...
-(void)imageButtonLoadedImage:(EGOImageButton *)imageView
{
    if (self.asTheNavi) {
        ...
        UIImage * h = [self.gg boxblurImageWithBlur:0.9];
        [self.backImageView setImage:h forState:UIControlStateNormal];
        return;
    }
    self.gg = imageView.currentImage;
...
//    [self.scrollView addObserver:self forKeyPath:@"contentOffset" options:(NSKeyValueObservingOptionNew) context:Nil];
    ...
    self.scrollView.contentInset = UIEdgeInsetsMake(self.asTheNavi?([UIDevice currentDevice].systemVersion.floatValue >= 7.0?64:0):self.frame.size.height, 0 ,0, 0);
}
