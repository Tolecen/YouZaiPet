...
- (void)configureWithParentLayer:(UIView*)parent previewRect:(CGRect)preivewRect {
    ...
    if (![parent.layer.sublayers containsObject:_previewLayer]) {
...
//        image = [self cutImage:image];/*裁剪照片*/
...
- (void)switchGrid:(BOOL)toShow {
    ...
    CGFloat headHeight = _previewLayer.bounds.size.height - SC_APP_SIZE.width+44;
    CGFloat squareLength = SC_APP_SIZE.width;
