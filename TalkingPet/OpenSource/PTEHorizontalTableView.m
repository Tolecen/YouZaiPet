...
{
    NSLog(@"offset:%f,contentSize:%@",scrollView.contentOffset.y,NSStringFromCGSize(scrollView.contentSize));
    if (scrollView.contentSize.height-scrollView.contentOffset.y<=ScreenWidth) {
        if ([self.delegate respondsToSelector:@selector(shouldToLoadNextPage)]) {
            [self.delegate shouldToLoadNextPage];
        }
    }
}
