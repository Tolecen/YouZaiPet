...
@implementation PaiHangShuoShuoTableHelper
-(id)initWithController:(UIViewController *)thexController Tableview:(UITableView *)theTable SectionView:(UIView *)sectionV
{
    if (self = [super init]) {
        ...
        self.yueduArray = [NSMutableArray array];
...
//            }
        ...
        //        BaseViewController * bv = (BaseViewController *)self.theController;
        if (self.dataArray.count>0) {
            self.firPageLoaded = YES;
            NSArray * y = [[responseObject objectForKey:@"value"] objectForKey:@"list"];
            [self.yueduArray removeAllObjects];
            for (int i = 0; i < y.count; i++) {
                [self.yueduArray addObject:[y[i] objectForKey:@"popNum"]];
            }
...
-(NSMutableArray *)getModelArray:(NSArray *)array
{
    ...
    for (int i = 0; i<array.count; i++) {
        TalkingBrowse * talking = [[TalkingBrowse alloc] initWithHostInfo:[[array objectAtIndex:i] objectForKey:@"petalkDTO"]];
...
{
    //    self.canJudgePlay = YES;
//    TalkingBrowse * theTalking = self.dataArray[indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    TalkingBrowse * theTalking = self.dataArray[indexPath.row];
        ...
        static NSString *cellIdentifier = @"paihangCell";
        PaiHangShuoShuoTableViewCell*cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier ];
        if (cell == nil) {
            cell = [[PaiHangShuoShuoTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
            cell.delegate = self;
        ...
        if (self.dataArray.count>0) {
            ...
            long long numv = [self.yueduArray[indexPath.row] longLongValue];
            ...
            [cell.reduLabel setText:[NSString stringWithFormat:@"%@热度",bnStr]];
        }
...
-(void)ImagesLoadedCellIndex:(int)cellIndex
{
    if (cellIndex==currentCellIndex) {
        ...
        NSIndexPath * indexPathTo = [NSIndexPath indexPathForRow:cellIndex inSection:0];
        if ((self.uploadArray&&self.uploadArray.count>0)&&indexPathTo.row==0) {
            return;
        }
...
-(void)cellPlayAni:(UIScrollView *)scrollView
{
    ...
    for (int i = 0; i<self.dataArray.count; i++) {
        NSIndexPath * indexPathTo = [NSIndexPath indexPathForRow:i inSection:0];
...
{
    TalkingBrowse * talking = [self.dataArray objectAtIndex:indexPath.row];
    ...
    talkingDV.shouldRequestInfo = YES;
    //    talkingDV.theIndex = indexPath.row;
    talkingDV.talking = [self.dataArray objectAtIndex:indexPath.row];
