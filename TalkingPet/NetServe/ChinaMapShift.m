...
LocationM
LocationMMake(double lng, double lat) {
    LocationM loc; loc.lng = lng, loc.lat = lat; return loc;
}
...
{
    LocationM wgLoc = gcLoc;
    LocationM currGcLoc, dLoc;
    while (1) {
...
LocationM bd_encrypt(LocationM gcLoc)
{
    ...
    return LocationMMake(z * cos(theta) + 0.0065, z * sin(theta) + 0.006);
}
...
LocationM bd_decrypt(LocationM bdLoc)
{
    ...
    double z = sqrt(x * x + y * y) - 0.00002 * sin(y * x_pi);
    double theta = atan2(y, x) - 0.000003 * cos(x * x_pi);
    return LocationMMake(z * cos(theta), z * sin(theta));
}
