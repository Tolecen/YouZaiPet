...
//            AVAudioPlayer * pl = [[AVAudioPlayer alloc] initWithData:data1 error:nil];
            pl.delegate = self;
            pl.enableRate = YES;
//            [pl prepareToPlay];
            [pl play];
...
//            AVURLAsset *asset = [[AVURLAsset alloc] initWithURL:[NSURL fileURLWithPath:fileName] options:[NSDictionary dictionaryWithObjectsAndKeys: [NSNumber numberWithBool:YES], AVURLAssetPreferPreciseDurationAndTimingKey, nil]];
//            float t = CMTimeGetSeconds(asset.duration);
            
            
//            NSLog(@"realPlayDuration:%f,setting:%@,rate:%f,fileLength:%lu",pl.duration,pl.settings,pl.rate,data1.length);
            self.player = pl;
//            [[UIDevice currentDevice] setProximityMonitoringEnabled:YES];
            if ([self.delegate respondsToSelector:@selector(didAudioPlayerBeginPlay:)]) {
                [self.delegate didAudioPlayerBeginPlay:_player];
