...
- (id)initWithFrame:(CGRect)frame WithSendBtn:(BOOL)ifWith withDeleteBtn:(BOOL)dele
{
    ...
    if (self) {
        UIImageView *sixGridBGV=[[UIImageView alloc]initWithFrame:CGRectMake(-320, 0, ScreenWidth*4, 253)];//原来是253
        [sixGridBGV setBackgroundColor:[UIColor colorWithRed:0.9 green:0.9 blue:0.9 alpha:1]];
        
        //创建表情视图
        UIScrollView *i_emojiScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0,  0, ScreenWidth, 253)];//原来是227和253
        //设置表情列表scrollview属性
        ...
        [m_EmojiScrollView setContentSize:CGSizeMake(ScreenWidth*3,253)];//原来是253
        ...
        emojiBGV = [[UIView alloc] initWithFrame:CGRectMake(0, self.frame.size.height-26.5-10, ScreenWidth, 45.5+26.5+10)];
        emojiBGV.backgroundColor = [UIColor clearColor];
        [self addSubview:emojiBGV];
        if(ifWith){
             UIImageView * ebgv = [[UIImageView alloc] initWithFrame:CGRectMake(0, 26.5+10, ScreenWidth, 45.5)];
            ...
            [sendEmojiBtn setFrame:CGRectMake(ScreenWidth-12-71.5, 43.5, 71.5, 32)];
        ...
        if (dele) {
            ...
            [backEmojiBtn setFrame:CGRectMake(ScreenWidth-12-49.5, 5, 40.5, 23)];
...
//    [inPutView setFrame:CGRectMake(0, self.view.frame.size.height-227-inPutView.frame.size.height, 320, inPutView.frame.size.height)];
    ...
    m_EmojiScrollView.showsHorizontalScrollIndicator = NO;
    m_EmojiScrollView.showsVerticalScrollIndicator = NO;
    [m_EmojiScrollView setContentSize:CGSizeMake(ScreenWidth*(self.emojiArray.count/28+1),253)];//原来是253
    m_EmojiScrollView.backgroundColor = [UIColor clearColor];
    m_EmojiScrollView.scrollEnabled = YES;
    [self addSubview:m_EmojiScrollView];
    [self emojiView];
...
{
	//创建并初始化uipagecontrol
	m_Emojipc=[[UIPageControl alloc]initWithFrame:CGRectMake(20, 180, ScreenWidth-40, 20)];
	//设置背景颜色
...
-(void)emojiView
{
    for (int n = 0; n <self.emojiArray.count; n++) {
        UIButton *btn = [[UIButton alloc]init];
//        if (n<28) {
            [btn setFrame:CGRectMake(((ScreenWidth-7*30*(ScreenWidth/320))/8)*(n%7+1)+30*(ScreenWidth/320)*(n%7)+ScreenWidth*(n/28), ((n-28*(n/28))/7+1)*12+30*((n-28*(n/28))/7), 30*(ScreenWidth/320), 30*(ScreenWidth/320))];
//        }
//        else if(n>=28&&n<56)
...
//            [btn setFrame:CGRectMake(13.75*(n%7+1)+30*(n%7)+640, ((n-56)/7+1)*12+30*((n-56)/7), 30, 30)];
        ...
        [btn setTitle:self.emojiArray[n] forState:UIControlStateNormal];
...
//	NSString *i_transCharacter = [m_pEmojiDic objectForKey:[NSString stringWithFormat:@"%d",n+1]];
    [self.delegate selectedEmoji:selectedButton.titleLabel.text];
    //提示文字标签隐藏
	//判断输入框是否有内容，追加转义字符
